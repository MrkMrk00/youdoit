...
const readUserInput = async (): Promise<{
	...
	const stageChoices = ['production', 'beta'] as const
...
try {
	...
	const contentApiUrl = config({ path: `website/.env.${stage}` }).parsed?.NEXT_PUBLIC_API_URL ?? ''
	...
	const handleApiErrors = async (response: Response, errorPrefix: string) => {
		...
		const message = data.error || data.errors.map((error: any) => error.message).join(' ')
